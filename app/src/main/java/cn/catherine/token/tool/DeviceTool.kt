...
class DeviceTool {
    ...
    fun getMemoryInfo(tag: String) {
        ...
        LogTool.i(
            TAG, "[" + tag + "]\t 可用内存：" + Formatter.formatFileSize(BaseApplication.context, mi.availMem)
    ...
    fun getDeviceOSInfo(): String {
        ...
        LogTool.d(
            TAG, MessageConstants.MANUFACTURER + manufacturer,
    ...
    fun checkIsTV(): Boolean {
        val uiModeManager =
            BaseApplication.context.getSystemService(UI_MODE_SERVICE) as UiModeManager ?: return false
